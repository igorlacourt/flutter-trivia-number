...
void main() {
  ...
  group('GetTriviaForConcreteNumber', () {
    ...
    test('get data from the concrete use case', () async {
      //arrange
      setUpInputConverterSuccess();
      when(mockGetConcreteNumberTrivia(
              any)) //it's called like a constructor, but it is the call function inside the class
          .thenAnswer((_) async => Right(tNumberTrivia));
      //act
      bloc.dispatch(GetTriviaForConcreteNumber(tNumberString));
      await untilCalled(mockGetConcreteNumberTrivia(
          any)); // await the mockGetConcreteNumberTrivia being called
      //assert
      verify(mockGetConcreteNumberTrivia(Params(
          number:
              tNumberParsed))); //it's called like a constructor, but it is the call function inside the class
    });

    test('emit [Loading, Loaded] when data is gotten successfuly', () async {
      //arrange
      setUpInputConverterSuccess();
      when(mockGetConcreteNumberTrivia(
              any)) //it's called like a constructor, but it is the call function inside the class
